...
>(({ id, icon, label, variant = "default", isHovered = false, onSelect, onMouseEnter, onMouseLeave, ...props }, ref) => {
  ...
  const getBackgroundColor = () => {
    ...
    if (isHovered) return "var(--color-light-glass-10)";
    return "transparent";
  };
  ...
  const getTextColor = () => {
    ...
    return "var(--color-light-glass-70)";
  };
...
export const MenuContext = React.forwardRef<HTMLDivElement, MenuContextProps>(
  ({ jokis = [], options, className, style, ...props }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className={cn("flex flex-col rounded-2xl backdrop-blur-[40px]", className)}
        style={{
          width: "177px",
          backgroundColor: "var(--color-dark-glass-70)",
          border: "1px solid var(--color-light-glass-20)",
          padding: "8px",
          gap: "4px",
          // Light Big effect: inset shadow
          boxShadow: "0px 18px 36px 0px rgba(0, 0, 0, 0.22)",

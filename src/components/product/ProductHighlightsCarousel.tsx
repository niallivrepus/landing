...
export function ProductHighlightsCarousel({
  ...
  const syncCardsToScrollPosition = useCallback((updateActive = true, parallaxActive = false) => {
    ...
    const cardMetrics = cards.map((card) => {
      ...
      const easedProgress = Math.sign(rawProgress) * Math.pow(absProgress, 0.82);
      ...
      return { card, easedProgress, titleOpacity };
    });

    for (const { card, easedProgress, titleOpacity } of cardMetrics) {
      const shouldAnimateParallax = parallaxActive && !shouldReduceMotion;
      card.style.setProperty("--highlight-title-opacity", titleOpacity.toFixed(3));
      if (shouldAnimateParallax) {
        card.style.setProperty("--highlight-media-x", `${(-easedProgress * 32).toFixed(2)}px`);
      }
      card.style.setProperty(
        "--highlight-media-scale",
        "1.04",
      );
      card.style.setProperty(
        "--highlight-media-transition",
        shouldAnimateParallax ? "none" : "transform 260ms cubic-bezier(0.22, 1, 0.36, 1)",
      );
    }

    if (!parallaxActive) {
      const settlingCard = cardMetrics.find(({ card }) => {
        return Number(card.dataset.slideIndex) === closestIndex;
      })?.card;
      settlingCard?.style.setProperty("--highlight-media-x", "0px");
    }
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      frame = window.requestAnimationFrame(() => {
        frame = 0;
        syncCardsToScrollPosition(!suppressIntersectionRef.current, true);
      });
    };
    const onScrollEnd = () => {
      if (idleTimer) {

...
export function LegalLayout({
  ...
  return (
    <div className="landing-cinema min-h-screen bg-black text-light-space antialiased">
      <SiteTopBar />
      <div className="pt-14 md:pt-14">
      <header className="sticky top-14 z-50 border-b border-light-glass-10 bg-black/90 backdrop-blur-md md:top-14">
        <div className="mx-auto flex h-[52px] max-w-[1024px] items-center justify-between gap-4 px-4 md:h-14 md:px-6">
          <div className="flex min-w-0 items-center gap-3">
            <Link
              to="/legal"
              className="font-sans text-[21px] font-semibold tracking-tight text-light-space transition-colors hover:text-[var(--color-blue-4)]"
            >
              Legal
            </Link>
          </div>
          ...
          >
            {subNav.map((item) => (
              <Link
                key={item.href}
                to={item.href}
                className="shrink-0 text-[12px] text-light-space/70 transition-colors hover:text-[var(--color-blue-4)] hover:underline"
              >
                {item.label}
              </Link>
            ))}
...
export function LegalBreadcrumb({ items }: { items: { label: string; to?: string }[] }) {
  return (
    <div className="border-b border-light-glass-10 bg-black">
      <div className="mx-auto max-w-[1024px] px-4 py-3 md:px-6">

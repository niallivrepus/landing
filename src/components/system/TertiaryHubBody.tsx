...
export function TertiarySection({
  ...
  return (
    <section className={cn("py-12 last:pb-0 md:py-14", className)}>
      <div className="grid gap-5 md:grid-cols-[220px_minmax(0,1fr)] md:gap-8">
        <h2 className="font-sans text-[20px] font-semibold tracking-[0em] text-light-space light:text-zinc-950 md:text-[24px]">
          {title}
        </h2>
    ...
    <div className={cn("grid gap-4", gridClass, className)}>
      {links.map((link) => (
        <SiteLink
          ...
          href={link.href}
          className="block rounded-[24px] border border-light-space/[0.08] bg-white/[0.02] px-5 py-5 transition-colors hover:border-light-space/[0.16] hover:bg-white/[0.04] light:border-black/[0.1] light:bg-section-grey-light light:hover:border-black/[0.14] light:hover:bg-zinc-200"
        >
          <div className="font-sans text-[16px] font-semibold tracking-[0em] text-light-space light:text-zinc-950">
            {link.label}
          </div>
          {link.description ? (
  ...
  return (
    <section className={cn("py-14 md:py-16", className)}>
      <div className={cn(TERTIARY_PAGE_SHELL, "text-center")}>
        <h2 className="mx-auto max-w-[720px] font-sans text-[28px] font-semibold leading-[1.1] tracking-[0em] text-light-space light:text-zinc-950 md:text-[36px]">
          {title}
        </h2>

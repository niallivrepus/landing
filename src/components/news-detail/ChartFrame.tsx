...
export function ChartFrame({ title, subtitle, legend, footnote, children }: ChartFrameProps) {
  return (
    <figure className="relative rounded-xl border border-white/[0.08] bg-black px-4 pb-4 pt-12 md:px-6 md:pb-6 md:pt-14">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <figcaption className="min-w-0 text-left">
          ...
          {subtitle ? (
            <p className="mt-0.5 font-sans text-xs font-medium text-white/55">{subtitle}</p>
          ) : null}
        </figcaption>
        {legend ? <div className="flex flex-wrap items-center gap-4 text-xs text-white/70">{legend}</div> : null}
      </div>
      <div className="mt-4 h-[260px] w-full sm:h-[300px] md:h-[320px]">{children}</div>
      {footnote ? (
        <p className="mt-4 font-sans text-[13px] italic leading-relaxed text-white/45">{footnote}</p>
      ) : null}
    </figure>
  );
}

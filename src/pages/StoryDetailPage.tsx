...
import { StoryAudioBlock } from "../components/stories/StoryAudioBlock";
import { StoryPoemBlock } from "../components/stories/StoryPoemBlock";
...
function renderSection(section: StorySection, key: number, storySlug: string) {
  switch (section.kind) {
    case "prose":
    ...
    case "audio":
      return (
        <div key={key} className={`${CONTENT_SHELL_WIDE} py-6 md:py-8`}>
    ...
    case "poem":
      return (
        <div key={key} className={`${CONTENT_SHELL_WIDE} py-10 md:py-14`}>
          <div className={articleColumn}>
            <StoryPoemBlock stanzas={section.stanzas} variant="editorial" />
          </div>
        </div>
      );
    case "cta":
...
export function StoryDetailPage() {
  ...
  return (
    <MarketingPageFrame theme={resolvedTheme === "light" ? "light" : "dark"}>
      <>
        <StoryHero story={story} />
        <StoryGallery images={story.heroGallery} />
        {story.sections.map((s, i) => renderSection(s, i, story.slug))}

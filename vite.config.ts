...
export default defineConfig(({ mode }) => {
  ...
  const publicBlurbsFeedEnv = resolvePublicBlurbsFeedEnv(env);
  const publicPeopleSearchEnv = resolvePublicPeopleSearchEnv(env);
  const publicProfileDemoEnv = resolvePublicProfileDemoEnv(env);

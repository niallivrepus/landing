...
The `.dmg` is gitignored (large binary). Railway `www` Docker build curls the `macos-1.0.3` GitHub Release into `dist/downloads/Jokuh.dmg`. If that fetch fails, `GET /downloads/Jokuh.dmg` 302s to the same release so the marketing href never points at GitHub.

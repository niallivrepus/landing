...
| `CONTACT_SALES_TO_EMAIL` | Inbox for contact sales submissions. Defaults to `sean@sierri.com`. Supports comma-separated recipients. |
| `CONTACT_SALES_FROM_EMAIL` | Verified sender for Resend, e.g. `Jokuh <sean@sierri.com>`. |
...
**Production target: Railway + Supabase only** (not Vercel). Full checklist: [`docs/railway-deployment.md`](docs/railway-deployment.md).
...
Push `Dockerfile`, `railway.toml`, `server/railway.ts`, and **`vendor/gooey/`** from this repo (mirrored from jokuh-live via `scripts/sync-gooey-vendor.mjs`), then add custom domains in Railway. **Do not** use `railway up` from a laptop — `public/` is too large for CLI uploads; GitHub-triggered builds clone on Railway.

**Runtime:** Node serves `dist/` static assets, SPA fallback, app handoff redirects, and `/api/*` routes (blurbs feed, people search, profile demo, contact sales, etc.).

**Build env** (service **`www`**): `VITE_ORIGIN_APP=https://app.jokuh.com`, `VITE_COOKIE_DOMAIN=.jokuh.com`.

**Runtime env** (required for profile/blurbs): `SUPABASE_SERVICE_ROLE_KEY`, optionally `GROQ_API_KEY`, `RESEND_API_KEY`, `ELEVENLABS_API_KEY`.

Cutover checklist: point DNS at Railway **`www`**, verify site + `/api/public-blurbs-feed`, remove apex domains from any legacy Vercel project.

Local production smoke test:

```bash
pnpm build:landing && pnpm build:server
STATIC_ROOT=dist pnpm start:railway
```
